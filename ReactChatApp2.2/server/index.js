...
const io = new Server(server, {
  ...
  socket.on("send_message_Master",(messageData)=>{

    console.log("send_message_Master");

    console.log("lanearray");
    console.log(lanearray);

    lanearray.map((lane)=>{
    socket.to(lane).emit("receive_message_Master_Lane",{messageData,lane});
    });
    ...
    roomarray.map((room)=>{
    socket.to(room).emit("receive_message_Master_Room",{messageData,room});
    })
  ...
  socket.on("join_room",(data)=>{
    ...
    lanearray.push(data.lane);
    lanearray=[...new Set(lanearray)]
    console.log("lanearray");
    console.log(lanearray);
    
    roomarray.push(data.room);
    roomarray=[...new Set(roomarray)]
    console.log("roomarray");
    console.log(roomarray);
    ...
    // socket.to(data.lane).emit("showsomeonejoinedLane",(data,socket.username));
